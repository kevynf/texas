...
use texas_core::{
    buffer::rope_text::RopeText, command::FocusCommand, language::TexasLanguage,
    line_ending::LineEnding, mode::Mode, movement::Movement, selection::Selection,
    syntax::Syntax,
};
use texas_rpc::proxy::ProxyResponse;
...
use crate::{
    command::{
        CommandExecuted, CommandKind, InternalCommand, TexasCommand, WindowCommand,
    },
    db::TexasDb,
    editor::{
        EditorData,
...
impl PaletteData {
    ...
    /// Initialize the palette with all the available workspaces.
    ...
    fn get_languages(&self) {
        let langs = TexasLanguage::languages();
        let items = langs
            .iter()
        ...
        if let Some(item) = items.get(index) {
            match &item.content {
                PaletteItemContent::PaletteHelp { cmd } => {
                    ...
                    self.common.texas_command.send(cmd);
                }
                ...
                PaletteItemContent::Command { cmd } => {
                    self.common.texas_command.send(cmd.clone());
                }
                ...
                PaletteItemContent::Language { name } => {
                    ...
                    if name.is_empty() || name.to_lowercase().eq("plain text") {
                        ...
                        let lang = match TexasLanguage::from_name(name) {
                            Some(v) => v,
                            None => return,
                        };
                ...
                PaletteItemContent::SCMReference { name } => {
                    self.common
                        .texas_command
                        .send(crate::command::TexasCommand {
                        kind: CommandKind::Workbench(
                            crate::command::TexasWorkbenchCommand::CheckoutReference,
                        ),
