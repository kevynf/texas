...
use crate::workspace::TexasWorkspace;
...
pub const LOGO: &str = include_str!("../../icons/texas/texas.svg");
...
static DEFAULT_DARK_THEME_COLOR_CONFIG: Lazy<ColorThemeConfig> = Lazy::new(|| {
    let (_, theme) =
        TexasConfig::load_color_theme_from_str(DEFAULT_DARK_THEME).unwrap();
...
impl TexasConfig {
    pub fn load(workspace: &TexasWorkspace) -> Self {
        let config = Self::merge_config(workspace, None, None);
        let mut texas_config: TexasConfig = match config.try_deserialize() {
            Ok(config) => config,
            Err(error) => {
                error!("Failed to deserialize configuration file: {error}");
                DEFAULT_TEXAS_CONFIG.clone()
            }
        };

        texas_config.available_color_themes = Self::load_color_themes();
        texas_config.available_icon_themes = Self::load_icon_themes();
        texas_config.resolve_theme(workspace);

        texas_config.color_theme_list = texas_config
            .available_color_themes
            .values()
            .map(|(name, _)| name.clone())
            .sorted()
            .collect();
        texas_config.color_theme_list.sort();

        texas_config.icon_theme_list = texas_config
            .available_icon_themes
            .values()
            .map(|(name, _, _)| name.clone())
            .sorted()
            .collect();
        texas_config.icon_theme_list.sort();

        texas_config.wrap_style_list = im::vector![
            WrapStyle::None.to_string(),
            WrapStyle::EditorWidth.to_string(),
            WrapStyle::WrapWidth.to_string()
        ];

        texas_config.terminal.get_indexed_colors();
    ...
    fn merge_config(
        ...
        if let Some(path) = workspace.path.as_ref() {
            let path = path.join("./.texas/settings.toml");
            config = config::Config::builder()
                .add_source(config.clone())
                .add_source(config::File::from(path.as_path()).required(false))
                .build()
    ...
    fn default_texas_config() -> TexasConfig {
        let mut default_texas_config: TexasConfig =
        ...
        default_texas_config.color_theme = DEFAULT_DARK_THEME_COLOR_CONFIG.clone();
        default_texas_config.icon_theme = DEFAULT_ICON_THEME_ICON_CONFIG.clone();
        default_texas_config.resolve_colors(None);
        default_texas_config
    }

    fn resolve_theme(&mut self, workspace: &TexasWorkspace) {
        let default_texas_config = DEFAULT_TEXAS_CONFIG.clone();
        ...
        self.resolve_colors(Some(&default_texas_config));
        self.update_id();
    }
    ...
    pub fn set_color_theme(&mut self, workspace: &TexasWorkspace, theme: &str) {
    ...
    /// Note that this does not save the config.
    pub fn set_icon_theme(&mut self, workspace: &TexasWorkspace, theme: &str) {
    ...
    fn resolve_colors(&mut self, default_config: Option<&TexasConfig>) {
        ...
        let fg = self.color(TexasColor::EDITOR_FOREGROUND).to_rgba8();
        let bg = self.color(TexasColor::EDITOR_BACKGROUND).to_rgba8();
    ...
    pub fn files_svg(&self, paths: &[&Path]) -> (String, Option<Color>) {
        ...
        if let Some(svg) = svg {
            let color = if self.icon_theme.use_editor_color.unwrap_or(false) {
                Some(self.color(TexasColor::TEXAS_ICON_ACTIVE))
            } else {
                None
            };
            (svg, color)
    ...
    fn terminal_get_named_color(
        ...
        let (color, alpha) = match color {
            alacritty_terminal::vte::ansi::NamedColor::Cursor => {
                (TexasColor::TERMINAL_CURSOR, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Foreground => {
                (TexasColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Background => {
                (TexasColor::TERMINAL_BACKGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Blue => {
                (TexasColor::TERMINAL_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Green => {
                (TexasColor::TERMINAL_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Yellow => {
                (TexasColor::TERMINAL_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Red => {
                (TexasColor::TERMINAL_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::White => {
                (TexasColor::TERMINAL_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Black => {
                (TexasColor::TERMINAL_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Cyan => {
                (TexasColor::TERMINAL_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::Magenta => {
                (TexasColor::TERMINAL_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlue => {
                (TexasColor::TERMINAL_BRIGHT_BLUE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightGreen => {
                (TexasColor::TERMINAL_BRIGHT_GREEN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightYellow => {
                (TexasColor::TERMINAL_BRIGHT_YELLOW, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightRed => {
                (TexasColor::TERMINAL_BRIGHT_RED, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightWhite => {
                (TexasColor::TERMINAL_BRIGHT_WHITE, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightBlack => {
                (TexasColor::TERMINAL_BRIGHT_BLACK, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightCyan => {
                (TexasColor::TERMINAL_BRIGHT_CYAN, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightMagenta => {
                (TexasColor::TERMINAL_BRIGHT_MAGENTA, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::BrightForeground => {
                (TexasColor::TERMINAL_FOREGROUND, 1.0)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlack => {
                (TexasColor::TERMINAL_BLACK, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimRed => {
                (TexasColor::TERMINAL_RED, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimGreen => {
                (TexasColor::TERMINAL_GREEN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimYellow => {
                (TexasColor::TERMINAL_YELLOW, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimBlue => {
                (TexasColor::TERMINAL_BLUE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimMagenta => {
                (TexasColor::TERMINAL_MAGENTA, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimCyan => {
                (TexasColor::TERMINAL_CYAN, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimWhite => {
                (TexasColor::TERMINAL_WHITE, 0.66)
            }
            alacritty_terminal::vte::ansi::NamedColor::DimForeground => {
                (TexasColor::TERMINAL_FOREGROUND, 0.66)
            }
        };
