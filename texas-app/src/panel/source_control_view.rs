...
use texas_core::buffer::rope_text::RopeText;
use texas_rpc::source_control::FileDiff;
    ...
    let commit_message = {
        ...
        label(i18n.text_signal("git.commit-message")).style(move |s| {
            ...
            s.absolute()
                ...
                .color(config.color(TexasColor::EDITOR_DIM))
                        ...
                        s.margin_top(10.0)
                            ...
                            .active(|s| {
                                s.background(config.color(
                                    TexasColor::PANEL_HOVERED_ACTIVE_BACKGROUND,
                                ))
                            })
                            .selectable(false)
                    })
...
fn file_diffs_view(source_control: SourceControlData) -> impl View {
    ...
    let texas_command = source_control.common.texas_command;
    ...
    let view_fn = move |(path, (diff, checked)): (PathBuf, (FileDiff, bool))| {
        ...
        stack((
            ...
            container({
                svg(move || {
                    let svg = match &diff {
                        FileDiff::Modified(_) => TexasIcons::SCM_DIFF_MODIFIED,
                        FileDiff::Added(_) => TexasIcons::SCM_DIFF_ADDED,
                        FileDiff::Deleted(_) => TexasIcons::SCM_DIFF_REMOVED,
                        FileDiff::Renamed(_, _) => TexasIcons::SCM_DIFF_RENAMED,
                    };
                ...
                .style(move |s| {
                    ...
                    let color = match &diff_for_style {
                        FileDiff::Modified(_) => TexasColor::SOURCE_CONTROL_MODIFIED,
                        FileDiff::Added(_) => TexasColor::SOURCE_CONTROL_ADDED,
                        FileDiff::Deleted(_) => TexasColor::SOURCE_CONTROL_REMOVED,
                        FileDiff::Renamed(_, _) => {
                            TexasColor::SOURCE_CONTROL_MODIFIED
                        }
                    };
            ...
            let discard = move || {
                texas_command.send(TexasCommand {
                    kind: CommandKind::Workbench(
                        TexasWorkbenchCommand::SourceControlDiscardTargetFileChanges,
                    ),
        ...
        .style(move |s| {
            ...
            s.padding_left(10.0)
                ...
                .hover(|s| {
                    s.background(config.color(TexasColor::PANEL_HOVERED_BACKGROUND))
                })
        })
    };
