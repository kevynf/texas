...
use crate::{
    app::clickable_icon,
    command::{InternalCommand, TexasWorkbenchCommand},
    ...
    stack((
        scroll(dyn_stack(
            ...
            move |(index, tab)| {
                ...
                let _local_terminal = terminal.clone();
                ...
                let svg_string = move || TexasIcons::TERMINAL;
                stack((
                    container({
                        stack((
                            container(
        ...
        container(clickable_icon(
            || TexasIcons::ADD,
            move || {
                workbench_command.send(TexasWorkbenchCommand::NewTerminalTab);
            },
            || false,
            || false,
...
fn tab_secondary_click(
    ...
    menu = menu.entry(MenuItem::new(i18n.text("terminal.clear-all")).action(
        move || {
