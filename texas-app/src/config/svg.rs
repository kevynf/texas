...
const TEXAS_ICONS_DIR: Dir = include_dir!("$CARGO_MANIFEST_DIR/../icons/texas");
...
impl SvgStore {
    fn new() -> Self {
        ...
        svgs.insert("texas_logo".to_string(), LOGO.to_string());
    ...
    pub fn logo_svg(&self) -> String {
        self.svgs.get("texas_logo").unwrap().clone()
    }

    pub fn get_default_svg(&mut self, name: &str) -> String {
        if !self.svgs.contains_key(name) {
            let file = if name.starts_with("texas") {
                TEXAS_ICONS_DIR.get_file(name).unwrap()
            } else {
                CODICONS_ICONS_DIR
                    .get_file(name)
