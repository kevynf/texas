...
mod tests {
    ...
    use crate::{config::TexasConfig, workspace::TexasWorkspace};

    #[test]
    fn test_resolve() {
        // Mimicking load
        let workspace = TexasWorkspace::default();

        let config = TexasConfig::merge_config(&workspace, None, None);
        let mut texas_config: TexasConfig = config.try_deserialize().unwrap();
...
"##;
        ...
        texas_config.available_color_themes =
            [("test".to_string(), ("test".to_string(), test_theme_cfg))]
                .into_iter()
                .collect();
        // texas_config.available_icon_themes = Some(vec![]);
        texas_config.core.color_theme = "test".to_string();
