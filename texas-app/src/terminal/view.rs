...
use regex::Regex;
use texas_core::{mode::Mode, rope_text_pos::Position};
use texas_rpc::{proxy::ProxyRpcHandler, terminal::TermId};
...
impl TerminalView {
    ...
    fn paint_content(
        ...
        let term_bg = config.color(TexasColor::TERMINAL_BACKGROUND);
    ...
    fn paint_line_content(
        ...
        if let Some((c, x)) = line_content.cursor {
            ...
            let cursor_color = if mode == Mode::Terminal {
                config.color(TexasColor::TERMINAL_CURSOR)
            } else {
                config.color(TexasColor::EDITOR_CARET)
            };
            if self.is_focused {
        ...
        if let Some(error) = self.launch_error.get() {
            ...
            text_layout.set_text(
                &self
                    ...
                    .replace("{error}", &error),
                AttrsList::new(
                    attrs.color(config.color(TexasColor::EDITOR_FOREGROUND)),
                ),
                None,
            );
            cx.draw_text(
        ...
        if let Some(selection) = content.selection.as_ref() {
            ...
            for line in start_line..end_line + 1 {
                ...
                cx.fill(
                    ...
                    config.color(TexasColor::EDITOR_SELECTION),
                    0.0,
                );
            }
