...
pub struct TabsInfo {
    ...
    pub workspaces: Vec<TexasWorkspace>,
}
...
/// resized using your window manager. Normally Texas has only one
...
impl WindowData {
    pub fn new(
        ...
        if window_tabs.with_untracked(|window_tabs| window_tabs.is_empty()) {
            let window_tab = Rc::new(WindowTabData::new(
                cx,
                Arc::new(TexasWorkspace::default()),
                common.clone(),
            ));
            window_tabs.update(|window_tabs| {
                window_tabs.push_back((cx.create_rw_signal(0), window_tab));
    ...
    pub fn info(&self) -> WindowInfo {
        let workspaces: Vec<TexasWorkspace> = self
            .window_tabs
            .get_untracked()
            .iter()
