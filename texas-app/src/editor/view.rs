...
use crate::{
    ...
    config::{TexasConfig, color::TexasColor, editor::WrapStyle, icon::TexasIcons},
    doc::DocContent,
...
fn editor_wrap(config: &TexasConfig) -> WrapMethod {
...
pub fn editor_style(
    ...
    .set(CursorColor, config.color(TexasColor::EDITOR_CARET))
    .set(SelectionColor, config.color(TexasColor::EDITOR_SELECTION))
    .set(
        CurrentLineColor,
        config.color(TexasColor::EDITOR_CURRENT_LINE),
    )
    .set(
        VisibleWhitespaceColor,
        config.color(TexasColor::EDITOR_VISIBLE_WHITESPACE),
    )
    .set(
        IndentGuideColor,
        config.color(TexasColor::EDITOR_INDENT_GUIDE),
    )
    .set(ScrollBeyondLastLine, config.editor.scroll_beyond_last_line)
    .color(config.color(TexasColor::EDITOR_FOREGROUND))
    .set(TextColor, config.color(TexasColor::EDITOR_FOREGROUND))
    .set(PhantomColor, config.color(TexasColor::EDITOR_DIM))
    .set(PlaceholderColor, config.color(TexasColor::EDITOR_DIM))
    .set(
        PreeditUnderlineColor,
        config.color(TexasColor::EDITOR_FOREGROUND),
    )
...
impl EditorView {
            ...
            match section.kind {
                ...
                DiffSectionKind::Added => {
                    cx.fill(
                        ...
                        config
                            .color(TexasColor::SOURCE_CONTROL_ADDED)
                            .multiply_alpha(0.2),
                        0.0,
                    );
                }
                DiffSectionKind::Removed => {
                    cx.fill(
                        ...
                        config
                            .color(TexasColor::SOURCE_CONTROL_REMOVED)
                            .multiply_alpha(0.2),
                        0.0,
                    );
                }
    ...
    fn paint_diff_no_code(
            ...
            .step_by(8)
        {
            ...
            if height > 0.0 {
                ...
                cx.stroke(
                    &Line::new(p0, p1),
                    config.color(TexasColor::EDITOR_DIM),
                    &Stroke::new(1.0),
                );
            }
        }
    ...
    fn paint_current_line(
        ...
        let breakline: Option<usize> = None;
    ...
    fn paint_find(&self, cx: &mut PaintCx, screen_lines: &ScreenLines) {
        ...
        let config = config.get_untracked();
        let line_height = config.editor.line_height() as f64;
        let color = config.color(TexasColor::EDITOR_FOREGROUND);
    ...
    fn paint_sticky_headers(
        ...
        cx.fill(
            &sticky_area_rect,
            config.color(TexasColor::EDITOR_STICKY_HEADER_BACKGROUND),
            0.0,
        );
    ...
    fn paint_scroll_bar(
        ...
        cx.fill(
            &Rect::ZERO
                ...
                .inflate(0.0, 10.0),
            config.color(TexasColor::TEXAS_SCROLL_BAR),
            0.0,
        );
    ...
    fn paint_scope_lines(
        ...
        let config = self.editor.common.config.get_untracked();
        let line_height = config.editor.line_height() as f64;
        let brush = config.color(TexasColor::EDITOR_FOREGROUND);
...
pub fn editor_container_view(
    ...
    stack((
        ...
        stack((
            ...
            empty().style(move |s| {
                ...
                s.absolute()
                    ...
                    // .border_bottom(1.0)
                    // .border_color(
                    //     config.get_color(TexasColor::TEXAS_BORDER),
                    // )
                    .apply_if(
        ...
        .style(|s| s.width_full().flex_basis(0).flex_grow(1.0_f32)),
    ))
    .on_cleanup(move || {
...
fn editor_gutter(
    ...
    let _viewport = ed.viewport;
    let scroll_delta = ed.scroll_delta;
    let _screen_lines = ed.screen_lines;

    let gutter_rect = create_rw_signal(Rect::ZERO);
    let _gutter_width = create_memo(move |_| gutter_rect.get().width());
                        ...
                        move |(i, section)| {
                            stack((
                                svg(move || {
                                    config
                                        .get()
                                        .ui_svg(TexasIcons::BREADCRUMB_SEPARATOR)
                                })
                                .style(move |s| {
                                    let config = config.get();
...
fn search_editor_view(
    ...
    stack((
        ...
        clickable_icon(
            || TexasIcons::SEARCH_CASE_SENSITIVE,
            move || {
                ...
                clickable_icon(
                    move || {
                        if replace_active.get() {
                            TexasIcons::ITEM_OPENED
                        } else {
                            TexasIcons::ITEM_CLOSED
                        }
                    },
                    move || {
    ...
    changes.iter().map(move |change| {
        ...
        let color = match change {
            DiffLines::Left(_range) => {
                Some(config.color(TexasColor::SOURCE_CONTROL_REMOVED))
            }
            DiffLines::Right(_range) => {
                ...
                if modified {
                    Some(config.color(TexasColor::SOURCE_CONTROL_MODIFIED))
                } else {
                    Some(config.color(TexasColor::SOURCE_CONTROL_ADDED))
                }
            }
            _ => None,
        };
