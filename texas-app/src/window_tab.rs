...
use crate::{
    ...
    command::{
        CommandExecuted, CommandKind, InternalCommand, TexasCommand,
        TexasWorkbenchCommand, WindowCommand,
    },
    config::TexasConfig,
    db::TexasDb,
    doc::DocContent,
    ...
    workspace::{TexasWorkspace, WorkspaceInfo},
};
...
pub struct CommonData {
    pub i18n: I18n,
    pub workspace: Arc<TexasWorkspace>,
    pub scope: Scope,
    ...
    pub texas_command: Listener<TexasCommand>,
    pub workbench_command: Listener<TexasWorkbenchCommand>,
    ...
    pub config: ReadSignal<Arc<TexasConfig>>,
...
pub struct WindowTabData {
    ...
    pub proxy: ProxyData,
    pub set_config: WriteSignal<Arc<TexasConfig>>,
...
impl WindowTabData {
    ...
    pub fn new(
        ...
        let config = TexasConfig::load(&workspace);
        let i18n = I18n::new(cx, &config.ui.language);
        let texas_command = Listener::new_empty(cx);
...

        {
            ...
            window_tab_data.common.texas_command.listen(move |cmd| {
                window_tab_data.run_texas_command(cmd);
            });
        }
    ...
    pub fn reload_config(&self) {
        let config = TexasConfig::load(&self.workspace);
    ...
    pub fn run_texas_command(&self, cmd: TexasCommand) {
        match cmd.kind {
    ...
    pub fn run_workbench_command(
        ...
        match cmd {
            ...
            ZoomReset => {
                ...
                TexasConfig::update_file("ui", "scale", toml_edit::Value::from(1.0));
            }
            ...
            SourceControlDiscardActiveFileChanges => {
                ...
                let diff = active_path.and_then(|path| {
                    self.source_control.file_diffs.with_untracked(|diffs| {
                        diffs.get(&path).map(|(diff, _)| diff.clone())
                    })
                });
                ...
                self.common
                    ...
                    .send(InternalCommand::ShowAlert {
                        ...
                        buttons: vec![AlertButton {
                            text: i18n
                                .text("dialog.discard-workspace-changes.confirm"),
                            action: Rc::new(move || {
                            ...
                            std::thread::Builder::new()
                                ...
                                .spawn(move || {
                                    let do_update = || -> anyhow::Result<()> {
                                        let src = crate::update::download_release(
                                            &release,
                                        )?;

                                        let path = crate::update::extract(
                                            &src,
                                            &process_path,
                                        )?;
