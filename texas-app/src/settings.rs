...
use texas_core::{buffer::rope_text::RopeText, mode::Mode};

use crate::{
    command::CommandExecuted,
    config::{
        DropdownInfo, TexasConfig, color::TexasColor, core::CoreConfig,
        editor::EditorConfig, icon::TexasIcons, terminal::TerminalConfig,
        ui::UIConfig,
    },
    i18n::I18n,
...
impl KeyPressFocus for SettingsData {
    fn get_mode(&self) -> texas_core::mode::Mode {
        Mode::Insert
    }
    ...
    fn run_command(
        &self,
        _command: &crate::command::TexasCommand,
        _count: Option<usize>,
        _mods: Modifiers,
    ...
    .style(move |s| {
        ...
        s.padding_horiz(20.0)
            ...
            .apply_if(kind == current_kind.get(), |s| {
                s.background(config.color(TexasColor::PANEL_CURRENT_BACKGROUND))
            })
            .hover(|s| {
            ...
            if let Some(is_ticked) = is_ticked {
                ...
                create_effect(move |last| {
                    ...
                    if let Ok(value) = serde::Serialize::serialize(
                        ...
                        TexasConfig::update_file(&kind, &field, value);
                    }
                });
                        ...
                        label(common.i18n.text_signal("common.reset"))
                            ...
                            .style(move |s| {
                                ...
                                s.margin_left(10)
                                    ...
                                    .border_radius(6)
                                    .border_color(
                                        config.color(TexasColor::TEXAS_BORDER),
                                    )
...
fn dropdown_view(
    ...
    stack((
        ...
        container(
            svg(move || {
                if expanded.get() {
                    config.get().ui_svg(TexasIcons::CLOSE)
                } else {
                    config.get().ui_svg(TexasIcons::DROPDOWN_ARROW)
                }
            })
            .style(move |s| {
                let config = config.get();
