...
use texas_core::selection::Selection;
use texas_rpc::{
...
fn file_node_text_color(
    ...
    let color = match diff {
        ...
        Some(FileDiffKind::Added) => TexasColor::SOURCE_CONTROL_ADDED,
        Some(FileDiffKind::Deleted) => TexasColor::SOURCE_CONTROL_REMOVED,
        None => TexasColor::PANEL_FOREGROUND,
    };
                        ...
                        move |s| {
                            s.height(ui_line_height.get())
                                .color(
                                    config
                                        .get()
                                        .color(TexasColor::PANEL_FOREGROUND_DIM),
                                )
                                .selectable(false)
                        },
                    ),
            ...
            stack((
                ...
                label(move || err.clone()).style(move |s| {
                    ...
                    let editor_background_color =
                        config.color(TexasColor::PANEL_CURRENT_BACKGROUND);
                    let error_background_color =
                        config.color(TexasColor::ERROR_LENS_ERROR_BACKGROUND);
                    ...
                    s.position(Position::Absolute)
                        ...
                        .width_full()
                        .color(config.color(TexasColor::ERROR_LENS_ERROR_FOREGROUND))
                ...
                let view = stack((
                    svg(move || {
                        ...
                        let svg_str = match open {
                            true => TexasIcons::ITEM_OPENED,
                            false => TexasIcons::ITEM_CLOSED,
                        };
                    ...
                    }),
                    {
                        ...
                        svg(move || {
                            ...
                            if is_dir {
                                let svg_str = match open {
                                    true => TexasIcons::DIRECTORY_OPENED,
                                    false => TexasIcons::DIRECTORY_CLOSED,
                                };
                                config.ui_svg(svg_str)
                        ...
                        s.padding_right(15.0)
                            ...
                            .apply_if(
                                ...
                                |x| {
                                    x.background(
                                        config.get().color(
                                            TexasColor::PANEL_CURRENT_BACKGROUND,
                                        ),
                                    )
                                },
                            )
