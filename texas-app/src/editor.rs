...
use crate::{
    command::{CommandKind, InternalCommand, TexasCommand, TexasWorkbenchCommand},
    config::TexasConfig,
    db::TexasDb,
    doc::{Doc, DocContent},
...
impl EditorData {
    ...
    fn run_edit_command(&self, cmd: &EditCommand) -> CommandExecuted {
        ...
        let yank_data =
            if let texas_core::cursor::CursorMode::Visual { .. } = &cursor.mode {
    ...
    fn run_multi_selection_command(
        ...
        // This is currently special-cased in Texas because floem editor does not have 'find'
        match cmd {
    ...
    fn run_move_command(
        &self,
        movement: &texas_core::movement::Movement,
        count: Option<usize>,
        mods: Modifiers,
    ) -> CommandExecuted {
        if movement.is_jump()
    ...
    fn inline_find(&self, direction: InlineFindDirection, c: &str) {
        ...
        if let Some(new_index) = match direction {
            ...
            self.run_move_command(
                &texas_core::movement::Movement::Offset(
    ...
    fn on_screen_find(&self, pattern: &str) -> Vec<SelRegion> {
        ...
        if let Some((_, offset, _)) = items.first().copied() {
            self.run_move_command(
                &texas_core::movement::Movement::Offset(offset),
                None,
                Modifiers::empty(),
            );
        }
    ...
    fn search_whole_word_forward(&self, mods: Modifiers) {
        ...
        if let Some((start, _end)) = next {
            self.run_move_command(
                &texas_core::movement::Movement::Offset(start),
                None,
                mods,
            );
        }
    ...
    pub fn word_at_cursor(&self) -> String {
        ...
        let region = self.cursor().with_untracked(|c| match &c.mode {
            texas_core::cursor::CursorMode::Normal(offset) => {
                texas_core::selection::SelRegion::caret(*offset)
            }
            texas_core::cursor::CursorMode::Visual {
                start,
                end,
                mode: _,
            } => texas_core::selection::SelRegion::new(
                *start.min(end),
                doc.buffer.with_untracked(|buffer| {
            ...
            texas_core::cursor::CursorMode::Insert(selection) => {
    ...
    fn right_click(&self, pointer_event: &PointerInputEvent) {
        ...
        let mut cmds = if is_file {
            vec![
                Some(CommandKind::Workbench(TexasWorkbenchCommand::RevealInPanel)),
                Some(CommandKind::Workbench(
                    TexasWorkbenchCommand::RevealInFileExplorer,
                )),
                Some(CommandKind::Workbench(
                    TexasWorkbenchCommand::SourceControlOpenActiveFileRemoteUrl,
                )),
                None,
                ...
                None,
                Some(CommandKind::Workbench(
                    TexasWorkbenchCommand::PaletteCommand,
                )),
            ]
        } else {
            vec![
        ...
        if self.diff_editor_id.get_untracked().is_some() && is_file {
            cmds.push(Some(CommandKind::Workbench(
                TexasWorkbenchCommand::GoToLocation,
            )));
        }
        let texas_command = self.common.texas_command;
        for cmd in cmds {
            if let Some(cmd) = cmd {
                menu = menu.entry(
                    MenuItem::new(
