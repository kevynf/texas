...
use texas_core::{directory::Directory, meta};
...
pub fn get_latest_release() -> Result<ReleaseInfo> {
    let url = match meta::RELEASE {
        ...
        meta::ReleaseType::Nightly => {
            "https://api.github.com/repos/texas/texas/releases/tags/nightly"
        }
        _ => "https://api.github.com/repos/texas/texas/releases/latest",
    };

    let resp = texas_proxy::get_url(url, Some("Texas"))?;
...
pub fn download_release(release: &ReleaseInfo) -> Result<PathBuf> {
    ...
    let name = "Texas-windows-portable.zip";
...
pub fn extract(src: &Path, process_path: &Path) -> Result<PathBuf> {
    ...
    std::fs::rename(process_path, dst_parent.join("texas.exe.bak"))?;
    std::fs::copy(parent.join("texas.exe"), process_path)?;
