...
pub enum SaveEvent {
    App(AppInfo),
    Workspace(TexasWorkspace, WorkspaceInfo),
    RecentWorkspace(TexasWorkspace),
    Doc(DocInfo),
...
impl TexasDb {
    ...
    pub fn recent_workspaces(&self) -> Result<Vec<TexasWorkspace>> {
        let workspaces =
            std::fs::read_to_string(self.folder.join(RECENT_WORKSPACES))?;
        let workspaces: Vec<TexasWorkspace> = serde_json::from_str(&workspaces)?;
        Ok(workspaces)
    }

    pub fn update_recent_workspace(&self, workspace: &TexasWorkspace) -> Result<()> {
    ...
    fn insert_recent_workspace(&self, workspace: TexasWorkspace) -> Result<()> {
...
fn workspace_folder_name(workspace: &TexasWorkspace) -> String {
