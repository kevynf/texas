        ...
        let injection_callback = |language: &InjectionLanguageMarker| {
            let language = match language {
                InjectionLanguageMarker::Name(name) => {
                    TexasLanguage::from_name(name)
                }
                InjectionLanguageMarker::Filename(path) => {
                    TexasLanguage::from_path_raw(path)
                }
                InjectionLanguageMarker::Shebang(id) => TexasLanguage::from_name(id),
            };
            language
...
impl Syntax {
    pub fn init(path: &Path) -> Syntax {
        let language = TexasLanguage::from_path(path);
        Syntax::from_language(language)
    }

    pub fn plaintext() -> Syntax {
        Self::from_language(TexasLanguage::PlainText)
    }

    pub fn from_language(language: TexasLanguage) -> Syntax {
    ...
    pub fn find_enclosing_pair(&self, offset: usize) -> Option<(usize, usize)> {
        if self.language == TexasLanguage::Markdown {
