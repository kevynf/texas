...
/// indices into the LANGUAGES array.  See method `TexasLanguage::properties`.
///
...
/// `TexasLanguage` as they will be accessed using the enum variants as indices.
const LANGUAGES: &[SyntaxProperties] = &[
    // Undetected/unmatched fallback or just plain file
    SyntaxProperties {
        id: TexasLanguage::PlainText,
        indent: Indent::tab(),
        files: &[],
        extensions: &["txt"],
    ...
    SyntaxProperties {
        id: TexasLanguage::Gitattributes,
        indent: Indent::tab(),
        files: &[],
        extensions: &[],
        comment: comment_properties!(),
    ...
    SyntaxProperties {
        id: TexasLanguage::KotlinBuildScript,
        indent: Indent::space(2),
        files: &[],
        extensions: &["kts"],
        comment: CommentProperties {
    ...
    SyntaxProperties {
        id: TexasLanguage::MarkdownInline,
        indent: Indent::space(4),
    ...
    SyntaxProperties {
        id: TexasLanguage::OcamlInterface,
        indent: Indent::space(2),
        files: &[],
        extensions: &["mli"],
        comment: comment_properties!("(*"),
    ...
    SyntaxProperties {
        id: TexasLanguage::SshClientConfig,
        indent: Indent::tab(),
        files: &[],
        extensions: &[],
        comment: comment_properties!(),
...
impl TexasLanguage {
    ...
    pub fn from_path(path: &Path) -> TexasLanguage {
        Self::from_path_raw(path).unwrap_or(TexasLanguage::PlainText)
    }

    pub fn from_path_raw(path: &Path) -> Option<TexasLanguage> {
    ...
    pub fn from_name(name: &str) -> Option<TexasLanguage> {
        match TexasLanguage::from_str(name.to_lowercase().as_str()) {
            Ok(v) => Some(v),
            Err(e) => {
                event!(Level::DEBUG, "failed parsing `{name}` TexasLanguage: {e}");
                None
            }
        }
    }
    ...
    fn get_grammar(&self) -> Option<tree_sitter::Language> {
        ...
        if let Some(grammars_dir) = Directory::grammars_directory() {
            match self::load_grammar(&grammar_name, &grammar_fn_name, &grammars_dir)
            {
                ...
                Err(err) => {
                    if self != &TexasLanguage::PlainText {
...
mod tests {
    ...
    use super::TexasLanguage;

    #[test]
    fn test_lanaguage_from_path() {
        let l = TexasLanguage::from_path(&PathBuf::new().join("test.rs"));
        assert_eq!(l, TexasLanguage::Rust);
    }
}
